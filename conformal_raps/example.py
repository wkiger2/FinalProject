...
# parser.add_argument('--batch_size', metavar='BSZ', help='batch size', default=10)
...
if __name__ == "__main__":
    ...
    root = r'C:\Users\WilliamKiger\Desktop\TEST\conformal_classification\conformal_classification-master\imagenet_val'
    validate_path = os.path.join(root, 'val')
    imagenet_calib_data, imagenet_val_data = torch.utils.data.random_split(torchvision.datasets.ImageFolder(validate_path, transform), [10000,50000-10000])

    # Initialize loaders 
    calib_loader = torch.utils.data.DataLoader(imagenet_calib_data, batch_size=10, shuffle=True, pin_memory=True)
    val_loader = torch.utils.data.DataLoader(imagenet_val_data, batch_size=10, shuffle=True, pin_memory=True)
